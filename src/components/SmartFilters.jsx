export default function SmartFilters({ value, onChange, variant = "sidebar" }) {
  const set = (patch) => onChange({ ...value, ...patch });
  const inDrawer = variant === "drawer";

  const Chip = ({ active, onClick, children, touch = false }) => (
    <button
      type="button"
      onClick={onClick}
      className={`px-3 rounded-full border text-sm ${touch ? "py-2 min-h-[40px]" : "py-1.5"}
  ...
  const shell =
    inDrawer
      ? "relative z-auto bg-transparent border-0 rounded-none px-0 py-0"
      : "sticky top-[52px] z-30 bg-white/70 backdrop-blur-md border border-slate-200 rounded-xl px-3 sm:px-4 py-3";
        ...
        <div className="flex flex-col gap-5">
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              <Chip touch active={value.level === "basic"} onClick={() => set({ level: value.level === "basic" ? "all" : "basic" })}>Basic</Chip>
              <Chip touch active={value.level === "standard"} onClick={() => set({ level: value.level === "standard" ? "all" : "standard" })}>Standard</Chip>
              <Chip touch active={value.level === "pro"} onClick={() => set({ level: value.level === "pro" ? "all" : "pro" })}>TOP</Chip>
            </div>
          </div>

          <div className="flex flex-col gap-2">
            <span className="text-sm font-medium text-slate-600">Ocena</span>
            <div className="flex flex-wrap gap-2">
              <Chip touch active={value.rating === 4} onClick={() => set({ rating: value.rating === 4 ? 0 : 4 })}>4★+</Chip>
              <Chip touch active={value.rating === 4.5} onClick={() => set({ rating: value.rating === 4.5 ? 0 : 4.5 })}>4.5★+</Chip>
            </div>
          </div>

          <div className="flex flex-col gap-1">
            <span className="text-sm font-medium text-slate-600">Termin</span>
            <select className="input w-full min-h-[44px]" value={value.eta} onChange={(e) => set({ eta: e.target.value })}>
          ...
          <div className="flex flex-col gap-2">
            <span className="text-sm font-medium text-slate-600">Budżet (zł)</span>
            <div className="flex items-center gap-2">
              <input type="number" className="input min-h-[44px] flex-1 min-w-0" min={0} value={value.budgetMin} onChange={(e) => set({ budgetMin: +e.target.value })} />
              <span className="text-slate-400">—</span>
              <input type="number" className="input min-h-[44px] flex-1 min-w-0" min={0} value={value.budgetMax} onChange={(e) => set({ budgetMax: +e.target.value })} />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 pt-3 border-t border-slate-200">
            <span className="text-sm font-medium text-slate-600">Sortowanie</span>
            <select className="input w-full min-h-[44px]" value={value.sort} onChange={(e) => set({ sort: e.target.value })}>
    ...
    <div className={shell}>
      <div className="flex flex-wrap items-center gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <input type="number" className="w-20 input" min={0} value={value.budgetMin} onChange={(e) => set({ budgetMin: +e.target.value })} />
          <span className="text-slate-400">—</span>
          <input type="number" className="w-20 input" min={0} value={value.budgetMax} onChange={(e) => set({ budgetMax: +e.target.value })} />
