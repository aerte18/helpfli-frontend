...
function HelpfliIcon({ sizeClassName, gradientId }) {
  return (
    <svg viewBox="0 0 48 48" className={sizeClassName} aria-hidden="true" focusable="false">
      <defs>
...
export default function OriginalLogoIcon({ className = "w-6 h-6", withBackground = false }) {
  const gradientId = useId();

  if (!withBackground) {
    return <HelpfliIcon sizeClassName={className} gradientId={gradientId} />;
  }
  ...
  return (
    <div
      ...
      aria-hidden="true"
    >
      <HelpfliIcon sizeClassName="w-3/4 h-3/4" gradientId={gradientId} />
    </div>
  );
}
